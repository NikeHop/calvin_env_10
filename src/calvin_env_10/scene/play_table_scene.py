...
from calvin_env_10.scene.objects.button import Button
from calvin_env_10.scene.objects.door import Door
from calvin_env_10.scene.objects.fixed_object import FixedObject
from calvin_env_10.scene.objects.light import Light
from calvin_env_10.scene.objects.movable_object import MovableObject
from calvin_env_10.scene.objects.switch import Switch
...
class PlayTableScene:
    def __init__(self, objects, data_path, euler_obs, p, cid, global_scaling, surfaces, np_random, **kwargs):
        ...
        # Convert objects to dict if it's not already a dict
        if hasattr(objects, 'to_container'):
            self.object_cfg = objects.to_container()
        else:
            self.object_cfg = dict(objects) if objects is not None else {}
